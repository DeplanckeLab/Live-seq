...
df_l_NT <-
  as.matrix(GetAssayData(live_raw, slot = "counts")[, live_raw$celltype_treatment == "Raw264.7_not_treated"])
df_l_LPS <-
  as.matrix(GetAssayData(live_raw, slot = "counts")[, live_raw$celltype_treatment == "Raw264.7_LPS_treated"])
prop_NT <- rowSums(df_l_NT > 2) / ncol(df_l_NT)
prop_LPS <- rowSums(df_l_LPS > 2) / ncol(df_l_LPS)

liveSeq_edgeR_raw$table$pct_NT <-
  prop_NT[rownames(liveSeq_edgeR_raw$table)]
liveSeq_edgeR_raw$table$pct_LPS <-
  prop_LPS[rownames(liveSeq_edgeR_raw$table)]
liveSeq_edgeR_raw$table$high_pct <- F
liveSeq_edgeR_raw$table$high_pct[liveSeq_edgeR_raw$table$pct_NT > 0.15 |
                                   liveSeq_edgeR_raw$table$pct_LPS > 0.15] <- T

saveRDS(
  liveSeq_edgeR_raw,
  "~/SVRAW1/prainer/FluidFM_exp5_Wanze/Participation_Manuscript_2021-22/Figures_Liveseq/DE-EdgeR_and_Downsampling/FULLData/RAW/liveseq_edgeR_res_RAW-Not-vs-LPSTreated.rds"
)
saveRDS(
  scRNA_edgeR_res_raw,
  "~/SVRAW1/prainer/FluidFM_exp5_Wanze/Participation_Manuscript_2021-22/Figures_Liveseq/DE-EdgeR_and_Downsampling/FULLData/RAW/scRNAeq_edgeR_res_RAW-Not-vs-LPSTreated.rds"
)

saveRDS(
  liveSeq_edgeR_raw,
  paste0(root_dir, "/data/liveseq_edgeR_res_RAW_NTvsLPSTreated.rds")
)
saveRDS(
  scRNA_edgeR_res_raw,
  paste0(root_dir, "/data/scRNAeq_edgeR_res_RAW_NTvsLPSTreated.rds")
)
...
P_t <-
  f_plot_CT(DE_res_live = liveSeq_edgeR_raw$table, DE_res_sc = scRNA_edgeR_res_raw$table) #9964
