...
df_l_NT <- as.matrix(GetAssayData(live_aspcs, slot = "counts")[,live_aspcs$celltype_treatment == "ASPC_not_treated"])
df_l_DMI <- as.matrix(GetAssayData(live_aspcs, slot = "counts")[,live_aspcs$celltype_treatment == "ASPC_DMIR_treated"])
prop_NT <- rowSums(df_l_NT > 2)/ncol(df_l_NT)
prop_DMI <- rowSums(df_l_DMI > 2)/ncol(df_l_DMI)

liveSeq_edgeR_aspc$table$pct_NT <- prop_NT[rownames(liveSeq_edgeR_aspc$table)]
liveSeq_edgeR_aspc$table$pct_DMI <- prop_DMI[rownames(liveSeq_edgeR_aspc$table)]
...
P_t <- f_plot_CT(liveSeq_edgeR_aspc$table, scRNA_edgeR_res_aspc$table) #10845
# P_t$lm_AllGenes
