...
##---------------------------------------------##
# GO enrichment
GOenrichment <- function(selectedGenes,
                         allGenesList,
                         ont = "BP",
                         topNodes = 10,
                         algo_Ranks = "eLimFisher",
  ...
  tg.1 <- new(
    ...
    ontology =  ont,
    allGenes = allGenesList_bg,
    geneSel = function (x) {
      return (x < 0.05)
    },
    annot = annFUN.org ,
    nodeSize = topNodes ,
    # minimum number of genes in a GO categorie
    ID = "ENSEMBL",
    mapping = Mapping
  )
  #GO.resKS <- runTest(tg.1, algorithm = "classic", statistic = "ks")
  GO.resF <-
    runTest(tg.1, algorithm = "classic", statistic = "fisher")
  #GO.resKS.elim <- runTest(tg.1, algorithm = "elim", statistic = "ks")
  GO.resF.elim <-
    runTest(tg.1, algorithm = "elim", statistic = "fisher")
  
  result <-
    GenTable(
      tg.1,
      classicFisher = GO.resF,
      eLimFisher = GO.resF.elim,
      orderBy = algo_Ranks,
      ranksOf = algo_Ranks,
      topNodes = topNodes
    )
...
GO_BP_aspcs_onlyL <-
  GOenrichment(
    selectedGenes = rownames(liveSeq_edgeR_aspc$table)[liveSeq_edgeR_aspc$table$Detected_as_DE_sc == "Only_Liveseq"],
    allGenesList = rownames(liveSeq_edgeR_aspc$table),
    ont = "BP"
  )
GO_CC_aspcs_onlyL <-
  GOenrichment(
...
GO_BP_aspcs_onlysc <-
  GOenrichment(
    selectedGenes = rownames(liveSeq_edgeR_aspc$table)[liveSeq_edgeR_aspc$table$Detected_as_DE_sc == "Only_sc"],
    allGenesList = rownames(liveSeq_edgeR_aspc$table),
    ont = "BP"
  )
GO_CC_aspcs_onlysc <-
...
GO_BP_raw_onlyL <-
  GOenrichment(
    selectedGenes = rownames(liveSeq_edgeR_raw$table)[liveSeq_edgeR_raw$table$Detected_as_DE_sc == "Only_Liveseq"],
    allGenesList = rownames(liveSeq_edgeR_raw$table),
    ont = "BP"
  )
GO_CC_raw_onlyL <-
  GOenrichment(
...
GO_BP_raw_onlysc <-
  GOenrichment(
    selectedGenes = rownames(liveSeq_edgeR_raw$table)[liveSeq_edgeR_raw$table$Detected_as_DE_sc == "Only_sc"],
    allGenesList = rownames(liveSeq_edgeR_raw$table),
    ont = "BP"
  )
GO_CC_raw_onlysc <-
