...
heatmap_and_save <-
  function(enr_res,
           ...
           n_top = 10,
           Term_to_select = NULL) {
    enrich.DBP <- lapply(enr_res, function(x)
      return(x[[dbp]]))
    if (!is.null(Term_to_select)) {
      enrich.DBP.top <- Term_to_select
    } else{
      enrich.DBP.top <-
        unique(unlist(lapply(enrich.DBP, function(x)
          return(x$Term[1:n_top]))))
    }
    
    enrich.DBP <- rbindlist(enrich.DBP)
    
    enrich.DBP.dcast <-
      dcast(enrich.DBP, Term ~ cat, value.var = "Adjusted.P.value")
    enrich.DBP.dcast[is.na(enrich.DBP.dcast)]  <-
      1  ## the NA value is filled with 1
...
liveseq_edgeR_res <-
  readRDS(file = paste0(root_dir, "/data/Liveseq_edgeR_Clust-vs-Rest.rds"))
scRNA_edgeR_res <-
  readRDS(file = paste0(root_dir, "/data/scRNAseq_edgeR_Clust-vs-Rest.rds"))
...
top100_degenes <-
  lapply(liveseq_edgeR_res, function(x) {
    x$table[x$table$is_de == T, "Name"][1:100]
  })
# enrichR
enrichR_res_live <-
  lapply(top100_degenes, function(x)
    enrichr(x, dbs))
# Add category (which group of cells)
enrichR_res_live <-
  lapply(names(enrichR_res_live), function(x) {
    out <- enrichR_res_live[[x]]
...
### scRNA-SEQ
top100_degenes <-
  lapply(scRNA_edgeR_res, function(x) {
    x$table[x$table$is_de == T, "Name"][1:100]
  })
...
enrichR_res_sc <-
  lapply(top100_degenes, function(x)
    enrichr(x, dbs))
enrichR_res_sc <-
  lapply(names(enrichR_res_sc), function(x) {
    out <- enrichR_res_sc[[x]]
...
live_MGA <-
  heatmap_and_save(enrichR_res_live, dbp = "Mouse_Gene_Atlas", n_top = 6)

## Extended data figure 2 t
sc_MGA <-
  heatmap_and_save(enrichR_res_sc, dbp = "Mouse_Gene_Atlas", n_top = 6)
...
live_BP_f <-
  heatmap_and_save(
    enrichR_res_live,
    dbp = "GO_Biological_Process_2015",
    Term_to_select = c(
...
sc_BP_f <-
  heatmap_and_save(
    ...
    Term_to_select = c(
      ...
      "regulation of homeostatic process (GO:0032844)",
      "extracellular matrix organization (GO:0030198)",
      "extracellular structure organization (GO:0043062)",
