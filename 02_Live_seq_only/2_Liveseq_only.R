...
# read all Seu.all
Seu.all <-
  readRDS(file.path(root_dir, "01_preprocessing/Seu.all.rds"))
# subset Liveseq
Liveseq <-
  subset(Seu.all,
...
gene.ribo <-
  Liveseq@assays$RNA@meta.features$ensembl_gene_id[grep("^Rp[s,l]",
                                                        Liveseq@assays$RNA@meta.features$external_gene_name)]
...
# creat seurat object
Liveseq <-
  CreateSeuratObject(counts = data.count, meta.data = Liveseq@meta.data)
# add the meta feature
Liveseq@assays$RNA@meta.features <-
  Seu.all@assays$RNA@meta.features[rownames(Liveseq),]
# add uniquely.mapped.rate and intron.mapped.rate in meta data
Liveseq@meta.data$uniquely.mapped.rate <-
  Liveseq@meta.data$uniquely.mapped / Liveseq@meta.data$input.reads
Liveseq@meta.data$intron.mapped.rate <-
  Liveseq@meta.data$nCount_RNA / Liveseq@meta.data$uniquely.mapped
# Liveseq_sub@meta.data$celltype_treatment  <-  droplevels(Liveseq_sub@meta.data$celltype_treatment  )
Liveseq@meta.data$celltype_treatment  <-
  factor(
    Liveseq@meta.data$celltype_treatment ,
    levels = c(
      "ASPC_not_treated",
      "ASPC_DMIR_treated",
      "IBA_not_treated",
      "Raw264.7_not_treated",
      "Raw264.7_LPS_treated"
    )
  )
...
ggplot(Liveseq@meta.data, aes(input.reads, nFeature_RNA)) +
  ...
  labs(
    title = paste(
      round(fit.lm$coefficients[2] * 1000000, 4) ,
      "(1M scale up) x+",
      round(fit.lm$coefficients[1], 4),
      "\n R2=",
...
plot2 <-
  LabelPoints(
    plot = plot1,
    points = top30,
    labels = Liveseq@assays$RNA@meta.features[top30, "external_gene_name"] ,
    repel = TRUE
  )
plot2
...
g2m.genes <-
  readRDS(file.path(root_dir, "data/g2m.genes.mouse.rds"))
...
## scale all the genes
Liveseq <-
  ScaleData(Liveseq, verbose = FALSE, features = rownames(Liveseq))
...
Liveseq <-
  FindClusters(Liveseq, resolution = 0.2)     ## resolution 0.3


p1 <- DimPlot(Liveseq, reduction = "tsne")
p2 <-
  DimPlot(Liveseq, reduction = "tsne", group.by = "celltype_treatment")
p3 <- DimPlot(Liveseq, reduction = "tsne", group.by = "Batch")
p4 <-
  FeaturePlot(Liveseq, reduction = "tsne", features  = "nFeature_RNA")
plot_grid(p1, p2, p3, p4)

p1 <- DimPlot(Liveseq, reduction = "umap")
p2 <-
  DimPlot(Liveseq, reduction = "umap", group.by = "celltype_treatment")
p3 <- DimPlot(Liveseq, reduction = "umap", group.by = "Batch")
p4 <-
  FeaturePlot(Liveseq, reduction = "umap", features  = "nFeature_RNA")
plot_grid(p1, p2, p3, p4)


p1 <- DimPlot(Liveseq, reduction = "pca")
p2 <-
  DimPlot(Liveseq, reduction = "pca", group.by = "celltype_treatment")
p3 <- DimPlot(Liveseq, reduction = "pca", group.by = "Batch")
p4 <-
  FeaturePlot(Liveseq, reduction = "pca", features  = "nFeature_RNA")
plot_grid(p1, p2, p3, p4)
...
## scale all the genes
Liveseq_ASPC <-
  ScaleData(Liveseq_ASPC,
            verbose = FALSE,
            features = rownames(Liveseq_ASPC))
## run PCA
...
Liveseq_ASPC <-
  FindNeighbors(Liveseq_ASPC, dims = 1:8)  # , k.param =15
Liveseq_ASPC <-
  FindClusters(Liveseq_ASPC, resolution = 0.3)     ## resolution 0.3
## also try cluster with k.mean
cluster.kmeans <-
  kmeans(t(Liveseq_ASPC@assays$RNA@data[VariableFeatures(Liveseq_ASPC) , ]),  2)
...
p2 <-
  DimPlot(Liveseq_ASPC, reduction = "tsne", group.by = "celltype_treatment")
p3 <- DimPlot(Liveseq_ASPC, reduction = "tsne", group.by = "Batch")
p4 <-
  FeaturePlot(Liveseq_ASPC, reduction = "tsne", features  = "nFeature_RNA")
p5 <-
  DimPlot(Liveseq_ASPC, reduction = "tsne", group.by = "kmean.2")
plot_grid(p1, p2, p3, p4, p5)

## get the subcluster cell name
subcluster0 <-
  names(Idents(Liveseq_ASPC) [Idents(Liveseq_ASPC) == "0"])
subcluster1 <-
  names(Idents(Liveseq_ASPC) [Idents(Liveseq_ASPC) == "1"])
...
Liveseq$subcluster <-
  factor(Liveseq$subcluster, levels = c(1, 2, 3, 4, 5))
...
gene.pseudogene <-
  geneName[endsWith(as.character(geneName$gene_biotype),  "pseudogene"), ]
gene.pseudoRemoved <-
  subset(
    Liveseq@assays$RNA@meta.features,
    !(ensembl_gene_id %in% gene.pseudogene$ensembl_gene_id)
  )
...
Liveseq.markers <-
  FindAllMarkers(
    ...
    features = rownames(gene.pseudoRemoved)
  )

# add gene symbol
Liveseq.markers$genesymbol <-
  Liveseq@assays$RNA@meta.features[Liveseq.markers$gene , "external_gene_name"]

# heatmap with top 10 DE genes
top10.ASPC <-
  Liveseq.markers %>% subset(cluster == 1 &
                               avg_log2FC > 0) %>% top_n(n = -10, wt = p_val_adj)
top10.ASPC_DMIR <-
...
# add genesymbol
Liveseq_condition.markers$genesymbol <-
  Liveseq@assays$RNA@meta.features[Liveseq$gene , "external_gene_name"]

# plot heatmap of top 10 DE genes
top10.ASPC <-
  Liveseq_condition.markers %>% subset(cluster == "ASPC_not_treated" &
                                         avg_log2FC > 0) %>% top_n(n = -10, wt = p_val_adj)
top10.ASPC_DMIR <-
  Liveseq_condition.markers %>% subset(cluster == "ASPC_DMIR_treated" &
                                         avg_log2FC > 0) %>% top_n(n = -10, wt = p_val_adj)
top10.IBA <-
  Liveseq_condition.markers %>% subset(cluster == "IBA_not_treated" &
                                         avg_log2FC > 0) %>% top_n(n = -10, wt = p_val_adj)
top10.RAW <-
  Liveseq_condition.markers %>% subset(cluster == "Raw264.7_not_treated" &
                                         avg_log2FC > 0) %>% top_n(n = -10, wt = p_val_adj)
top10.RAW_LPS <-
  Liveseq_condition.markers %>% subset(cluster == "Raw264.7_LPS_treated" &
                                         avg_log2FC > 0) %>% top_n(n = -10, wt = p_val_adj)
top10 <-
  rbind(top10.ASPC,
        top10.ASPC_DMIR,
...
write.csv(
  Liveseq_condition.markers,
  "02_Live_seq_only/DEs.Liveseq_per_celltype_treatment.csv"
)
...
Down_Sample_Seurat <-
  function(pbmc.data.filter,
           Meta_data ,
           expected_depth)
  {
    print(paste0("start downsampling to ", expected_depth, " reads", sep = ""))
    pbmc.data.down <-
      Down_Sample_Matrix(pbmc.data.filter,  expected_depth)
    ...
    Meta_data$sample_name <-
      factor(Meta_data$sample_name, levels = Meta_data$sample_name)
    ...
    prefix <-
      ifelse(colnames(Meta_data) %in% colnames(pbmcDown@meta.data),
             "ori_",
             "")
    print(prefix)
    colnames(Meta_data) <-
      paste(prefix, colnames(Meta_data), sep = "")
    ...
    pbmcDown@meta.data$cellType_condition <-
      paste(pbmcDown@meta.data$Cell_type,
            pbmcDown@meta.data$LPS_treatment,
            sep = "+")
    ...
    pbmcDown <-
      FindClusters(pbmcDown, resolution = 0.25)     ## resolution 0.3
    return(pbmcDown)
  }
...
registerDoSNOW(cl)
sampling.size <-
  c(500000, 250000, 100000, 50000, 25000, 10000, 5000, 2500, 1000, 500)
...
downsample.list <-
  foreach(
    i = 1:length(sampling.size),
    .packages = "Seurat",
    .export = c("Down_Sample_Matrix", "Down_Sample_Seurat"),
    .verbose = T
  ) %dopar% {
    sink("log.foreach.txt", append = TRUE)
    Down_Sample_Seurat(Liveseq@assays$RNA@counts[, colnames(Liveseq)],
                       Liveseq@meta.data,
                       sampling.size[i])
  }
...
p2 <-
  DimPlot(downsample.list[[2]] , reduction = "tsne" , group.by = "celltype_treatment")
plot_grid(p1, p2)

p1 <- DimPlot(downsample.list[[5]], reduction = "tsne")
p2 <-
  DimPlot(downsample.list[[5]] , reduction = "tsne" , group.by = "celltype_treatment")
plot_grid(p1, p2)
...
for (i in 1:length(downsample.new)) {
  ...
  p2 <-
    DimPlot(objectToPlot, reduction = "tsne", group.by = "celltype_treatment")
  print(plot_grid(p1, p2))
  #
  ...
  p <-
    ggplot(objectToPlot@meta.data,
           aes(x = ident, fill = celltype_treatment)) +
    geom_bar() +
    geom_text(
...
saveRDS(downsample.new,
        file.path(root_dir, "02_Live_seq_only/downsample.new.rds"))
