...
df <- data.frame(catalog=c("Total", "Library preparation", "Passing QC"), number=c(641,588, ncol(Liveseq)))
df$catalog <- factor(df$catalog , levels = df$catalog )
p <- ggplot(df, aes(catalog, number)) + geom_bar(stat = "identity") + xlab(label = "") + ylim(0,500)
p <- p + theme(
...
plist <- lapply(c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.rRNA", "percent.protein", "uniquely.mapped.rate", "intron.mapped.rate", "input.reads"), function(i) { 
  ...
  p <- ggplot(Liveseq@meta.data, aes_string(x="sampling_type", y=i)) + 
...
plist <- lapply(c("input.reads","uniquely.mapped.rate","intron.mapped.rate",  "nCount_RNA","nFeature_RNA", "percent.mt", "percent.rRNA", "percent.protein" ), function(x) {
  p <- VlnPlot(object = Liveseq, features = x, group.by = "celltype_treatment", cols = color.celltype )
...
# per batch
plist <- lapply(c("input.reads","uniquely.mapped.rate","intron.mapped.rate",  "nCount_RNA","nFeature_RNA", "percent.mt", "percent.rRNA", "percent.protein" ), function(x) {
  p <- VlnPlot(object = Liveseq, features = x, group.by = "Batch", cols = color.batch )
  p <- p + scale_x_discrete(labels= c("ASPC_Pre", "ASPC_Post", "IBA", "RAW_Mock", "RAW_LPS"))
  p <- p + xlab("")
  p <- p + ylab(ylab.metrics[x]) + ggtitle(title.metrics[x])
  return(p)
...
fit.inputRNA <-  lm( nFeature_RNA ~ input.RNA   , data= Liveseq@meta.data %>% filter(input.RNA < 5))

ggplot(Liveseq@meta.data %>% filter(input.RNA < 5), aes(input.RNA, nFeature_RNA)) + geom_point() + geom_smooth(method = lm) + 
  annotate("text", x= 2, y = 9000, label=paste( fit.inputRNA$coefficients[2], "x+",    fit.inputRNA$coefficients[1])  )
# ggsave("inputRNA.nGene.curve.pdf")


## explore the cells misclustered
Liveseq$idents <- Idents(Liveseq)
misClusterASPC_Mock <- Liveseq@meta.data %>% filter(idents != "1" & (celltype_treatment == "ASPC_not_treated"))
misClusterASPC_DMIR <- Liveseq@meta.data %>% filter(idents != "2" & (celltype_treatment == "ASPC_DMIR_treated" ))
misClusterIBA <-  Liveseq@meta.data %>% filter(idents != "3" & celltype_treatment == "IBA_not_treated")
misClusterRAW_Mock <-  Liveseq@meta.data %>% filter(idents != "4" & celltype_treatment == "Raw264.7_not_treated")
misClusterRAW_LPS <-  Liveseq@meta.data %>% filter(idents != "5" & celltype_treatment == "Raw264.7_LPS_treated")

misCLuster.all <- rbind(misClusterASPC_Mock, misClusterASPC_DMIR, misClusterIBA, misClusterRAW_Mock, misClusterRAW_LPS )


Liveseq$is.clusterOK <- "yes"
Liveseq$is.clusterOK [Liveseq$sample_name %in% misCLuster.all$sample_name  ] <- "no"

# cells_misClustered per nFeature
p.clusterOK.nFeature <- t.test(Liveseq@meta.data[Liveseq@meta.data$is.clusterOK == "no" , "nFeature_RNA"] , Liveseq@meta.data[Liveseq@meta.data$is.clusterOK == "yes" , "nFeature_RNA"] )
...
p <- ggplot(Liveseq@meta.data, aes(is.clusterOK, nFeature_RNA ))  + 
  ...
  stat_summary(fun.data=mean_sdl, fun.args = list(mult=1), geom="pointrange", color=viridis(10)[1]) +
  theme_classic() +
  annotate("text",x=1, y=9000,  label=paste("t test, p ",  p.clusterOK.nFeature$p.value))
...
p.clusterOK.input.RNA <- t.test(Liveseq@meta.data[Liveseq@meta.data$is.clusterOK == "no" , "input.RNA"] , Liveseq@meta.data[Liveseq@meta.data$is.clusterOK == "yes" , "input.RNA"] )
...
p <- ggplot(Liveseq@meta.data, aes(is.clusterOK, input.RNA ))  + 
  ...
  theme_classic() +
  ylim(0,8) +
  annotate("text",x=1, y=7,  label=paste("t test, p ",  p.clusterOK.input.RNA$p.value))
...
library(dendextend)
Liveseq.rename <- RenameCells(Liveseq, new.names =as.character(Liveseq@meta.data$original_sample_name) )
Liveseq.rename <- RenameCells(Liveseq, new.names =as.character(Liveseq@meta.data$sample_name) )

dend <- Liveseq.rename@assays$RNA@data[VariableFeatures(Liveseq.rename), ] %>% # get the most variable gene expression data
...
mycolor <-  droplevels.factor(df.temp$celltype_treatment)   ## color according to the level
...
## plot the color bar
df <- data.frame(sample.name =as.character(seq(from = 1,to = length(mycolor))), mycolor= as.character(mycolor) )
...
ASPC.cell <- colnames(subset(Liveseq, celltype_treatment == "ASPC_not_treated"))
ASPCtreated.cell <- colnames(subset(Liveseq, celltype_treatment == "ASPC_DMIR_treated"))
IBA.cell <- colnames(subset(Liveseq, celltype_treatment == "IBA_not_treated"))
RAWLpsN.cell <- colnames(subset(Liveseq, celltype_treatment %in% c("Raw264.7_not_treated")))
RAWLpsP.cell <- colnames(subset(Liveseq, celltype_treatment %in% c("Raw264.7_LPS_treated")))
...
## umap plots
p1 <- DimPlot(Liveseq, reduction = "umap", cols = color.celltype,label = TRUE,label.size = 2 ,repel = T, pt.size = 0.3) + NoLegend()

p2 <- DimPlot(Liveseq, reduction = "umap", group.by = "celltype_treatment",label = TRUE,label.size = 2 ,repel = T, pt.size = 0.3,cols = color.celltype) + NoLegend()

p3 <- FeaturePlot(Liveseq, reduction  = "umap", features = "nFeature_RNA",  pt.size = 0.3)

p4 <- DimPlot(Liveseq, reduction  = "umap", group.by = "Batch", cols = color.batch, pt.size = 0.3)
...
p2 <- DimPlot(Liveseq, reduction = "tsne", group.by = "celltype_treatment",label = TRUE,label.size = 2 ,repel = T, pt.size = 0.3, cols = color.celltype) + NoLegend()

p3 <- FeaturePlot(Liveseq, reduction  = "tsne", features = "nFeature_RNA", pt.size = 0.3)

p4 <- DimPlot(Liveseq, reduction  = "tsne", group.by = "Batch", cols = color.batch, pt.size = 0.3)
...
p1 <- DimPlot(Liveseq, reduction = "pca", cols = color.celltype,label = TRUE,label.size = 2 ,repel = T, pt.size = 0.3) + NoLegend()

p2 <- DimPlot(Liveseq, reduction = "pca", group.by = "celltype_treatment",label = TRUE,label.size = 2 ,repel = T, pt.size = 0.3, cols = color.celltype) + NoLegend()

p3 <- FeaturePlot(Liveseq, reduction  = "pca", features = "nFeature_RNA", pt.size = 0.3)

p4 <- DimPlot(Liveseq, reduction  = "pca", group.by = "Batch", cols = color.batch, pt.size = 0.3)
...
######## get the top genes
top10.ASPC <- Liveseq.markers %>% subset(cluster=="1" & avg_log2FC > 0 )%>% top_n(n = 20, wt = avg_log2FC)
top10.ASPC_DMIR <- Liveseq.markers %>% subset(cluster=="2" & avg_log2FC > 0 )%>% top_n(n = 20, wt = avg_log2FC)
top10.IBA <- Liveseq.markers %>% subset(cluster=="3" & avg_log2FC > 0 )%>% top_n(n = 20, wt = avg_log2FC)
top10.RAW <- Liveseq.markers %>% subset(cluster=="4" & avg_log2FC > 0 ) %>% top_n(n = 20, wt = avg_log2FC)
top10.RAW_LPS <- Liveseq.markers %>% subset(cluster=="5" & avg_log2FC > 0 )%>% top_n(n = 20, wt = avg_log2FC)
...
top10 <- top10  %>% group_by(cluster) %>%  top_n(n = 8, wt = avg_log2FC)
head(top10)
...
p <-DoHeatmap(Liveseq, features = as.character(top10$gene), group.colors = color.celltype) + 
...
p <- DoHeatmap(Liveseq, features = as.character(top10$gene) ) + NoLegend() + 
  scale_y_discrete(labels= rev(top10$genesymbol)) +
...
FeaturePlot(object = Liveseq, reduction = "umap", features = symbol.to.ensembl("Tnf") )

# Dimplot the DE gene
genesToPlot <- c("Dpep1", "Thy1", "Gpx3","Sfrp2" ,"Mycn","Tnf" )
plist <- lapply(genesToPlot, function(x) FeaturePlot(object = Liveseq, reduction = "umap", features = symbol.to.ensembl(x) ))
plot_grid(plotlist=plist)
# ggsave("gene_expression_umap.pdf", width = 7.2, height = 4, useDingbats=FALSE)

plist <- lapply(genesToPlot, function(x) FeaturePlot(object = Liveseq, reduction =  "tsne", features = symbol.to.ensembl(x)  ))
plot_grid(plotlist=plist)
...
for (i in levels(Liveseq$double_extraction)) {
  cells.selected <- names( Liveseq$double_extraction[ Liveseq$double_extraction == i] ) ## select double extraction cells
  treatment <- Liveseq$treatment[ Liveseq$double_extraction == i]           ## get the LPS treatment info of double extraction cells
  double_extraction_order <- Liveseq$double_extraction_order[ Liveseq$double_extraction == i]  ## get the sampling order (1st or 2nd sampling)
  ilabel <- paste(Liveseq@meta.data[cells.selected, "original_sample_name"], treatment, double_extraction_order,    collapse = " ")

  p<- DimPlot(Liveseq, reduction = "tsne", group.by = "double_extraction", cells.highlight = cells.selected,  sizes.highlight = 1.5, pt.size = 0.3) + 
    ggtitle(ilabel ) 
  plist[[i]] <- p
}
