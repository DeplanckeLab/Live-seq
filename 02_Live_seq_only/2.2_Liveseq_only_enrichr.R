...
levels(DE.liveseq.all$cluster ) <- c("ASPC_not_treated", "ASPC_DMIR_treated", "IBA_not_treated", "Raw264.7_not_treated", "Raw264.7_LPS_treated")
...
top100.liveseq.c1 <-  DE.liveseq.all %>% subset(cluster == "ASPC_not_treated"& pct.1/pct.2 > 3) %>% arrange(desc(avg_log2FC)) %>% top_n(100, wt = avg_log2FC)
top100.liveseq.c2 <-  DE.liveseq.all %>% subset(cluster == "ASPC_DMIR_treated"& pct.1/pct.2 > 3) %>% arrange(desc(avg_log2FC)) %>% top_n(100, wt = avg_log2FC)
top100.liveseq.c3 <-  DE.liveseq.all %>% subset(cluster == "IBA_not_treated"& pct.1/pct.2 > 3) %>% arrange(desc(avg_log2FC)) %>% top_n(100, wt = avg_log2FC)
top100.liveseq.c4 <-  DE.liveseq.all %>% subset(cluster == "Raw264.7_not_treated"& pct.1/pct.2 > 3) %>% arrange(desc(avg_log2FC)) %>% top_n(100, wt = avg_log2FC)
top100.liveseq.c5 <-  DE.liveseq.all %>% subset(cluster == "Raw264.7_LPS_treated"& pct.1/pct.2 > 3) %>% arrange(desc(avg_log2FC)) %>% top_n(100, wt = avg_log2FC)
...
write.csv(enrich_top5 , "enrichr.MGA.csv")
