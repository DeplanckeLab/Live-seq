...
f_downsample <- function(i,
                         dataset_to_sample = subset_sc_aspc,
...
p_orig <-
  density_plots(live_aspcs, sc_aspcs, sc_matrix = F, tittle = "Original data")
...
subset_sc_aspc <-
  subset_sc_aspc[!rownames(subset_sc_aspc) %in% c("EGFP", "mCherry"), ]
...
l_cell_ord <-
  sort(colSums(GetAssayData(live_aspcs, slot = "counts")[!rownames(live_aspcs) %in% c("EGFP", "mCherry"),] > 0))
...
down_meta_aspcs <-
  data.frame(
    row.names = colnames(down_sc_aspc),
    nCount = colSums(down_sc_aspc),
    nFeat = colSums(down_sc_aspc > 0)
  )
down_meta_aspcs$prop <-
  down_meta_aspcs$nCount / nCount_subset_aspc[rownames(down_meta_aspcs)]

p_down <-
  density_plots(live_aspcs, down_sc_aspc, tittle = "Down-sampled sc")

## UPSAMPLE
# Calculate nCount of live-seq cells without considering EGFP and mCherry
nCount_l <-
  colSums(GetAssayData(live_aspcs, slot = "counts")[!rownames(live_aspcs) %in% c("EGFP", "mCherry"),])

# Up sample
up_sc_aspc <-
  do.call(cbind, lapply(1:ncol(down_sc_aspc), function(x)
    upOrdown_sample(down_sc_aspc[, x, drop = F], nCount_l[names(l_cell_ord)[x]])))
colnames(up_sc_aspc) <- colnames(down_sc_aspc)

p_up <-
  density_plots(live_aspcs, up_sc_aspc, tittle = "Up-scaled sc")

## CREATE SEU OBJ OF DS scRNAseq
sc_aspcs_ds <- CreateSeuratObject(up_sc_aspc)
sc_aspcs_ds$celltype_treatment <-
  sc_aspcs$celltype_treatment[colnames(sc_aspcs_ds)]
...
sc_DownSampled_edgeR_res_aspc <-
  f_edgeR_CT(sc_aspcs_ds, genes_to_keep = g)
...
df_l_NT <-
  as.matrix(GetAssayData(sc_aspcs_ds, slot = "counts")[, sc_aspcs_ds$celltype_treatment == "ASPC_not_treated"])
df_l_DMI <-
  as.matrix(GetAssayData(sc_aspcs_ds, slot = "counts")[, sc_aspcs_ds$celltype_treatment == "ASPC_DMIR_treated"])
prop_NT <- rowSums(df_l_NT > 2) / ncol(df_l_NT)
prop_DMI <- rowSums(df_l_DMI > 2) / ncol(df_l_DMI)

sc_DownSampled_edgeR_res_aspc$table$pct_NT <-
  prop_NT[rownames(sc_DownSampled_edgeR_res_aspc$table)]
sc_DownSampled_edgeR_res_aspc$table$pct_DMI <-
  prop_DMI[rownames(sc_DownSampled_edgeR_res_aspc$table)]
sc_DownSampled_edgeR_res_aspc$table$high_pct <- F
sc_DownSampled_edgeR_res_aspc$table$high_pct[sc_DownSampled_edgeR_res_aspc$table$pct_NT > 0.15 |
                                               sc_DownSampled_edgeR_res_aspc$table$pct_DMI > 0.15] <- T
...
P_t <-
  f_plot_CT(liveSeq_edgeR_aspc$table,
            sc_DownSampled_edgeR_res_aspc$table) #10845
