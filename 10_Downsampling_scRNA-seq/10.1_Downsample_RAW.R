...
density_plots <- function(live,
  ...
  p_lib <- ggplot(df_lib, aes(x = lib, col = which)) +
    ...
    geom_vline(data = mu,
               aes(xintercept = grp.mean, color = which),
               linetype = "dashed") +
    mashaGgplot2Theme +
  ...
  df_feat <-
    data.frame(
      feat = c(live$nFeature_RNA, sc$nFeature_RNA),
      which = c(rep("live", ncol(live)), rep("sc", length(sc$nCount_RNA)))
    )
...
upOrdown_sample <- function(x, lib.size) {
  ...
  if (is.null(dim(x))) {
    ...
    d <- data.frame(ens_id = rownames(x), freq = x[, 1])
  }
  data.to.sample <- d %>% filter(freq != 0)
  data.to.sample <-
    unlist(sapply(1:nrow(data.to.sample), function(i)
      rep(data.to.sample[i, "ens_id"], data.to.sample[i, "freq"])))
  ...
  data.output <-
    data.frame(row.names = d$ens_id, value = rep(0, nrow(d)))
...
f_downsample <- function(i,
                         dataset_to_sample = subset_sc_raw,
                         nCount_origSC_all_cells = nCount_subset_raw,
                         cell_ord_sc = sc_cell_ord ,
                         cell_order_l = l_cell_ord) {
  cat(paste("CELL", i, ": goal nFeat", cell_order_l[i]))
  cat("\n")
  ...
  lib.size_test <-
    round((1 - (cell_ord_sc[i] - cell_order_l[i]) / cell_ord_sc[i]) * nCount_origSC_all_cells[names(cell_ord_sc[i])])
  ...
  while (counter < 50 & ok == F) {
    # Sample from sc data
    out <-
      upOrdown_sample(dataset_to_sample[, names(cell_ord_sc[i])], lib.size_test)
    ...
    if (abs(out_nFeat - cell_order_l[i]) < 5) {
      ok <- T
    }
    lib.size_test <-
      round((1 - (out_nFeat - cell_order_l[i]) / out_nFeat) * lib.size_test)
    counter <- counter + 1
  } # Max 50 iterations
  cat("\n")
...
o_orig <-
  density_plots(live_raw, sc_raw, sc_matrix = F, tittle = "Original data")
...
sample_NT_cells <-
  sample(colnames(sc_raw)[sc_raw$treatment == "not_treated"], 102)
sample_LPS_cells <-
  sample(colnames(sc_raw)[sc_raw$treatment == "LPS_treated"], 50)
...
subset_sc_raw <-
  GetAssayData(sc_raw, slot = "counts")[, c(sample_NT_cells, sample_LPS_cells)]
subset_sc_raw <-
  subset_sc_raw[!rownames(subset_sc_raw) %in% c("EGFP", "mCherry"), ]
...
l_cell_ord <-
  sort(colSums(GetAssayData(live_raw, slot = "counts")[!rownames(live_raw) %in% c("EGFP", "mCherry"),] > 0))

## DOWNSAMPLE
# Downsample and recreate count matrix
t <-
  lapply(1:length(sc_cell_ord), function(i)
    f_downsample(
      i,
      dataset_to_sample = subset_sc_raw,
      nCount_origSC_all_cells = nCount_subset_raw
...
down_meta_raw <-
  data.frame(
    row.names = colnames(down_sc_raw),
    nCount = colSums(down_sc_raw),
    nFeat = colSums(down_sc_raw > 0)
  )
down_meta_raw$prop <-
  down_meta_raw$nCount / nCount_subset_raw[rownames(down_meta_raw)]

p_down <-
  density_plots(live_raw, down_sc_raw, tittle = "Down-sampled sc")

## UPSAMPLE
# Calculate nCount of live-seq cells without considering EGFP and mCherry
nCount_l <-
  colSums(GetAssayData(live_raw, slot = "counts")[!rownames(live_raw) %in% c("EGFP", "mCherry"),])

# Up sample
up_sc_raw <-
  do.call(cbind, lapply(1:ncol(down_sc_raw), function(x)
    upOrdown_sample(down_sc_raw[, x, drop = F], nCount_l[names(l_cell_ord)[x]])))
...
sc_raw_ds$celltype_treatment <-
  sc_raw$celltype_treatment[colnames(sc_raw_ds)]
...
sc_DownSampled_edgeR_res_raw <-
  f_edgeR_CT(sc_raw_ds, genes_to_keep = g)
...
df_l_NT <-
  as.matrix(GetAssayData(sc_raw_ds, slot = "counts")[, sc_raw_ds$celltype_treatment == "Raw264.7_not_treated"])
df_l_LPS <-
  as.matrix(GetAssayData(sc_raw_ds, slot = "counts")[, sc_raw_ds$celltype_treatment == "Raw264.7_LPS_treated"])
prop_NT <- rowSums(df_l_NT > 2) / ncol(df_l_NT)
prop_LPS <- rowSums(df_l_LPS > 2) / ncol(df_l_LPS)

sc_DownSampled_edgeR_res_raw$table$pct_NT <-
  prop_NT[rownames(sc_DownSampled_edgeR_res_raw$table)]
sc_DownSampled_edgeR_res_raw$table$pct_LPS <-
  prop_LPS[rownames(sc_DownSampled_edgeR_res_raw$table)]
sc_DownSampled_edgeR_res_raw$table$high_pct <- F
sc_DownSampled_edgeR_res_raw$table$high_pct[sc_DownSampled_edgeR_res_raw$table$pct_NT > 0.15 |
                                              sc_DownSampled_edgeR_res_raw$table$pct_LPS > 0.15] <- T
...
P_t <-
  f_plot_CT(liveSeq_edgeR_raw$table, sc_DownSampled_edgeR_res_raw$table) #9964
