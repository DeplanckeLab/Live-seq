...
scRNAseq_only <- scRNAseq_only %>% FindClusters(res = 0.1)
...
scRNAseq_only$MODIFIED_RNA_snn_res.0.1 <- scRNAseq_only$RNA_snn_res.0.1
levels(scRNAseq_only$MODIFIED_RNA_snn_res.0.1) <- c("Raw264.7_not_treated", "IBA_not_treated", "Raw264.7_LPS_treated", "ASPC")
scRNAseq_only$MODIFIED_RNA_snn_res.0.1 <- as.character(scRNAseq_only$MODIFIED_RNA_snn_res.0.1)
scRNAseq_only$MODIFIED_RNA_snn_res.0.1[colnames(sc_aspcs_all)] <- as.character(sc_aspcs_all$RNA_snn_res.0.1)
...
grid.arrange(DimPlot(scRNAseq_only, group.by = "MODIFIED_RNA_snn_res.0.1", reduction = "tsne", 
                     cols = unname(celltype_treatment_colors[levels(as.factor(scRNAseq_only$MODIFIED_RNA_snn_res.0.1))])) + theme(legend.position = "none"),
             ...
             nrow = 1,
             top = paste0("ARI = ", round(mclust::adjustedRandIndex(scRNAseq_only$celltype_treatment, scRNAseq_only$MODIFIED_RNA_snn_res.0.1), 2)))
mclust::adjustedRandIndex(scRNAseq_only$celltype_treatment, scRNAseq_only$MODIFIED_RNA_snn_res.0.1) #0.9522529
...
df <- reshape2::melt(table(scRNAseq_only$celltype_treatment, paste0(scRNAseq_only$MODIFIED_RNA_snn_res.0.1, "-clust")))
df$Var1 <- factor(as.character(df$Var1), levels = names(celltype_treatment_colors))
df$Var2 <- factor(as.character(df$Var2), levels = paste0(names(celltype_treatment_colors), "-clust"))
ggplot(df, aes(Var2, value, fill = Var1)) + geom_bar(stat = "identity") + scale_fill_manual(values = unname(celltype_treatment_colors)) + 
  mashaGgplot2Theme + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ylab("Number of cells")
...
scRNAseq_only$MODIFIED_RNA_snn_res.0.1 <- factor(scRNAseq_only$MODIFIED_RNA_snn_res.0.1, levels = levels(scRNAseq_only$celltype_treatment))
...
scRNAseq_only$MODIFIED_RNA_snn_res.0.1_CellType <- scRNAseq_only$MODIFIED_RNA_snn_res.0.1; levels(scRNAseq_only$MODIFIED_RNA_snn_res.0.1_CellType) <- c(rep("ASPC", 2), "IBA", rep("RAW", 2))
...
#   #scRNA-seq
# t_sc_CT <- table(scRNAseq_only$Celltype_simple, scRNAseq_only$MODIFIED_RNA_snn_res.0.1_CellType)
...
Wrong_Live_noIBA <- sum(t_live_noIBA[row(t_live_noIBA) != col(t_live_noIBA)])/sum(t_live_noIBA) #0.076 
  #scRNA-seq
t_sc_noIBA <- table(scRNAseq_only$celltype_treatment[scRNAseq_only$celltype_treatment != "IBA_not_treated"], scRNAseq_only$MODIFIED_RNA_snn_res.0.1[scRNAseq_only$celltype_treatment != "IBA_not_treated"])
